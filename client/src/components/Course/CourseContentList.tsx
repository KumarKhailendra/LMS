        ...
        <div className={`mt-[15px] w-full ${!props.isDemo && 'ml-[-30px] min-h-screen sticky top-24 left-0 z-30'}`}>
            {
                videoSections.map((section: string, index: number) => {
                    ...
                    const sectionVideLength: number = sectionVideos.reduce((acc: number, item: any) => acc + item.videoLength, 0);
                    const sectionStartIndex: number = totalCount;
                    const sectionContentHours: number = sectionVideLength / 60;
                    totalCount += sectionVideoCount;
                    return (
                        <div className={`${!props.isDemo && 'border-b border-[#ffffff8e] pb-2'}`} key={index}>
                            ...
                            <h5 className="text-black dark:text-white">
                                ...
                                {sectionVideLength < 60 ? sectionVideLength : sectionContentHours.toFixed(2)}{" "}
                                {sectionVideLength < 60 ? "Minutes" : "Hours"}
                            </h5>
                            <br />
                            {
                                isSectionVisible && (
